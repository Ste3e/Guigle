...
class EntryBox{
	...
	public void build(void delegate(int a, string b) dg, int eId, string msg, string style, Cursor cursor){
		this.dgExt = dg;
		this.eId = eId;
		this.eText = msg;
		this.cursor = cursor;
		...
		int ex = wx + 50;
		int ey = wy + wh - 100;
		style=format("x:%s y:%s w:100 h:20 xoff:0 yoff:0", to!string(ex), to!string(ey));
		editField = new EditField(&dummyEvent, 0, style);
		
		//BUTTON
		int bx = wx + ww - 150;
		int by = wy + 50;
		style = format("text:OK x:%s y:%s w:100 h:20 xoff:30 yoff:2", to!string(bx), to!string(by));
	...
	public void dummyEvent(int event, int element, string data){
	...
	public void updateText(string newText){
		editField.updateText(newText);
	}
	
	//INSTANCE
	private static EntryBox instance;
	private this(){
		dat = Global.getInstance();
	}
	
	package static EntryBox getInstance(){
		if(instance is null){
			instance = new EntryBox();
		}
		return instance;
	} 
}
